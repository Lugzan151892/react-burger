        ...
        <div className={styles.container}>     
            ...
            <div className={styles.orders}>
                <div className={`${styles.orderlist} mr-15`}>
                    {
                        isUserAuth ? 
                        allOrders.reverse().map((el: TOrderElement) => (
                            <OrderListElement isProfile={false} item={el} key={el._id}/>
                        )) :
                        allOrders.map((el: TOrderElement) => (
                            <OrderListElement isProfile={false} item={el} key={el._id}/>
                        ))
                    }                
                </div>
                    ...
                    <div className={styles.orders_numbers}>
                        <div className={`${styles.orders_container} mr-9`}>
                            ...
                            <div className={styles.orders__numbersList}>
                                <div className='mr-4'>
                                    {
                                        readyOrders.slice(0, 10).map((el: TOrderElement, index: number)=> (
                                ...
                                </div>
                                {
                                    ...
                                    <div>
                                        {
                                            readyOrders.slice(10, 20).map((el: TOrderElement, index: number)=> (
                                                <p className="text text_type_digits-default mb-2" key={index}>{el.number}</p>
                                            ))
                                        }
                                    </div> 
                        ...
                        <div className={styles.orders_container}>
                            ...
                            <div className={styles.orders__numbersList}>
                                <div className='mr-4'>
                                    {
                                        preparingOrders.slice(0, 10).map((el: TOrderElement, index: number)=> (
                                            <p className="text text_type_digits-default mb-2" key={index}>{el.number}</p>
                                        ))
                                    }
                                </div>
                                {
                                    ...
                                    <div>
                                        {
                                            preparingOrders.slice(10, 20).map((el: TOrderElement, index: number)=> (
                                                <p className="text text_type_digits-default mb-2" key={index}>{el.number}</p>
                                            ))
                                        }
                                    </div> 
